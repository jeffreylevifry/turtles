...
(function($, window, document, undefined) {
	...
	Class.Methods = {
		addMethods: function(e) {
			...
			if (!Object.keys({
					toString: true
				}).length) n.push("toString", "valueOf");
	...
	b2PairManager.prototype = {
		...
		Commit: function() {
			...
			for (e = 0; e < this.m_pairBufferCount; ++e) {
				var r = this.Find(this.m_pairBuffer[e].proxyId1, this.m_pairBuffer[e].proxyId2);
				r.ClearBuffered();
				var i = n[r.proxyId1];
				var s = n[r.proxyId2];
				if (r.IsRemoved()) {
	...
	b2BroadPhase.prototype = {
		...
		ComputeBounds: function(e, t, n) {
			...
			e[0] = this.m_quantizationFactor.x * (r - this.m_worldAABB.minVertex.x) & b2Settings.USHRT_MAX - 1;
			t[0] = this.m_quantizationFactor.x * (s - this.m_worldAABB.minVertex.x) & 65535 | 1;
			e[1] = this.m_quantizationFactor.y * (i - this.m_worldAABB.minVertex.y) & b2Settings.USHRT_MAX - 1;
			t[1] = this.m_quantizationFactor.y * (o - this.m_worldAABB.minVertex.y) & 65535 | 1
		},
	...
	b2Collision.b2CollidePoly = function(e, t, n, r) {
		...
		I = b2Collision.ClipSegmentToLine(j, m, b2Collision.b2CollidePolyTempVec, H);
		if (I < 2) return;
		b2Collision.b2CollidePolyTempVec.Set(x, T);
		I = b2Collision.ClipSegmentToLine(F, j, b2Collision.b2CollidePolyTempVec, B);
		if (I < 2) return;
		if (p) {
			e.normal.Set(-L, -A)
		} else {
	...
	b2Collision.b2CollidePolyAndCircle = function(e, t, n, r) {
		...
		for (var v = 0; v < t.m_vertexCount; ++v) {
			var m = t.m_normals[v].x * (u - t.m_vertices[v].x) + t.m_normals[v].y * (a - t.m_vertices[v].y);
			if (m > d) {
				return
			}
			if (m > p) {
	...
	b2Shape.Destroy = function(e) {
		if (e.m_proxyId != b2Pair.b2_nullProxy) e.m_body.m_world.m_broadPhase.DestroyProxy(e.m_proxyId)
	};
	...
	b2Shape.PolyMass = function(e, t, n, r) {
		...
		for (var f = 0; f < n; ++f) {
			...
			var S = d.x;
			var x = d.y;
			var T = a * (.25 * (w * w + S * w + S * S) + (y * w + y * S)) + .5 * y * y;
			var N = a * (.25 * (E * E + x * E + x * x) + (b * E + b * x)) + .5 * b * b;
			o += v * (T + N)
		}
		e.mass = r * s;
		i.Multiply(1 / s);
	...
	b2ShapeDef.prototype = {
		...
		ComputeMass: function(e) {
			...
			switch (this.type) {
				case b2Shape.e_circleShape:
					{
						var t = this;e.mass = this.density * b2Settings.b2_pi * t.radius * t.radius;e.center.Set(0, 0);e.I = .5 * e.mass * t.radius * t.radius
					}
					break;
				case b2Shape.e_boxShape:
					{
						var n = this;e.mass = 4 * this.density * n.extents.x * n.extents.y;e.center.Set(0, 0);e.I = e.mass / 3 * b2Math.b2Dot(n.extents, n.extents)
					}
					break;
				case b2Shape.e_polyShape:
					{
						var r = this;b2Shape.PolyMass(e, r.vertices, r.vertexCount, this.density)
					}
					break;
				default:
					e.mass = 0;
 ...
 this.m_restitution = e.restitution;
			...
			var r = e;
			this.m_localPosition.Set(e.localPosition.x - n.x, e.localPosition.y - n.y);
			this.m_type = b2Shape.e_circleShape;
			this.m_radius = r.radius;
			this.m_R.SetM(this.m_body.m_R);
			var i = this.m_R.col1.x * this.m_localPosition.x + this.m_R.col2.x * this.m_localPosition.y;
			var s = this.m_R.col1.y * this.m_localPosition.x + this.m_R.col2.y * this.m_localPosition.y;
			...
			var o = new b2AABB;
			o.minVertex.Set(this.m_position.x - this.m_radius, this.m_position.y - this.m_radius);
			o.maxVertex.Set(this.m_position.x + this.m_radius, this.m_position.y + this.m_radius);
		...
		Synchronize: function(e, t, n, r) {
			this.m_R.SetM(r);
			this.m_position.x = r.col1.x * this.m_localPosition.x + r.col2.x * this.m_localPosition.y + n.x;
			this.m_position.y = r.col1.y * this.m_localPosition.x + r.col2.y * this.m_localPosition.y + n.y;
			if (this.m_proxyId == b2Pair.b2_nullProxy) {
				return
			}
			var i = e.x + (t.col1.x * this.m_localPosition.x + t.col2.x * this.m_localPosition.y);
			var s = e.y + (t.col1.y * this.m_localPosition.x + t.col2.y * this.m_localPosition.y);
		...
		QuickSync: function(e, t) {
			this.m_R.SetM(t);
			this.m_position.x = t.col1.x * this.m_localPosition.x + t.col2.x * this.m_localPosition.y + e.x;
			this.m_position.y = t.col1.y * this.m_localPosition.x + t.col2.y * this.m_localPosition.y + e.y
		},
		ResetProxy: function(e) {
			if (this.m_proxyId == b2Pair.b2_nullProxy) {
				return
			...
			t = null;
			var n = new b2AABB;
			n.minVertex.Set(this.m_position.x - this.m_radius, this.m_position.y - this.m_radius);
			n.maxVertex.Set(this.m_position.x + this.m_radius, this.m_position.y + this.m_radius);
			if (e.InRange(n)) {
		...
		Support: function(e, t, n) {
			...
			e /= r;
			t /= r;
			n.Set(this.m_position.x + this.m_radius * e, this.m_position.y + this.m_radius * t)
		},
	...
	Object.extend(b2PolyShape.prototype, {
		...
		initialize: function(e, t, n) {
			...
			if (e.type == b2Shape.e_boxShape) {
				...
				this.m_localCentroid.x = e.localPosition.x + (a.col1.x * p + a.col2.x * d) - n.x;
				this.m_localCentroid.y = e.localPosition.y + (a.col1.y * p + a.col2.y * d) - n.y;
				for (r = 0; r < this.m_vertexCount; ++r) {
					...
					this.m_coreVertices[r].x = this.m_vertices[r].x - 2 * b2Settings.b2_linearSlop * v;
					this.m_coreVertices[r].y = this.m_vertices[r].y - 2 * b2Settings.b2_linearSlop * m
				}
			}
			var y = Number.MAX_VALUE;
			...
			this.m_localOBB.center.Set((y + w) * .5, (b + E) * .5);
			this.m_localOBB.extents.Set((w - y) * .5, (E - b) * .5);
			var x = 0;
			var T = 0;
			for (r = 0; r < this.m_vertexCount; ++r) {
			...
			i = b2PolyShape.tAbsR.col1.x * this.m_localOBB.extents.x + b2PolyShape.tAbsR.col2.x * this.m_localOBB.extents.y;
			s = b2PolyShape.tAbsR.col1.y * this.m_localOBB.extents.x + b2PolyShape.tAbsR.col2.y * this.m_localOBB.extents.y;
			var N = this.m_position.x + (this.m_R.col1.x * this.m_localOBB.center.x + this.m_R.col2.x * this.m_localOBB.center.y);
			var C = this.m_position.y + (this.m_R.col1.y * this.m_localOBB.center.x + this.m_R.col2.y * this.m_localOBB.center.y);
			u.minVertex.x = N - i;
			u.minVertex.y = C - s;
			u.maxVertex.x = N + i;
			u.maxVertex.y = C + s;
		...
		Synchronize: function(e, t, n, r) {
			this.m_R.SetM(r);
			this.m_position.x = this.m_body.m_position.x + (r.col1.x * this.m_localCentroid.x + r.col2.x * this.m_localCentroid.y);
			this.m_position.y = this.m_body.m_position.y + (r.col1.y * this.m_localCentroid.x + r.col2.y * this.m_localCentroid.y);
			if (this.m_proxyId == b2Pair.b2_nullProxy) {
				return
			}
			var i;
			...
			i = this.syncMat.col1.x * this.m_localOBB.extents.x + this.syncMat.col2.x * this.m_localOBB.extents.y;
			s = this.syncMat.col1.y * this.m_localOBB.extents.x + this.syncMat.col2.y * this.m_localOBB.extents.y;
		...
		QuickSync: function(e, t) {
			this.m_R.SetM(t);
			this.m_position.x = e.x + (t.col1.x * this.m_localCentroid.x + t.col2.x * this.m_localCentroid.y);
			this.m_position.y = e.y + (t.col1.y * this.m_localCentroid.x + t.col2.y * this.m_localCentroid.y)
		},
		ResetProxy: function(e) {
			if (this.m_proxyId == b2Pair.b2_nullProxy) {
				return
		...
		Support: function(e, t, n) {
			...
			n.Set(this.m_position.x + (this.m_R.col1.x * this.m_coreVertices[s].x + this.m_R.col2.x * this.m_coreVertices[s].y), this.m_position.y + (this.m_R.col1.y * this.m_coreVertices[s].x + this.m_R.col2.y * this.m_coreVertices[s].y))
		},
	...
	b2Body.prototype = {
		SetOriginPosition: function(e, t) {
			...
			this.m_position = b2Math.AddVV(e, b2Math.b2MulMV(this.m_R, this.m_center));
		...
		ApplyForce: function(e, t) {
			if (this.IsSleeping() == false) {
				this.m_force.Add(e);
				this.m_torque += b2Math.b2CrossVV(b2Math.SubtractVV(t, this.m_position), e)
			}
		},
		...
		ApplyImpulse: function(e, t) {
			if (this.IsSleeping() == false) {
				...
				this.m_angularVelocity += this.m_invI * b2Math.b2CrossVV(b2Math.SubtractVV(t, this.m_position), e)
			}
		},
		GetMass: function() {
			return this.m_mass
		...
		initialize: function(e, t) {
			...
			for (n = 0; n < this.m_shapeCount; ++n) {
				...
				i = s[n];
				this.m_I += i.I;
				var o = b2Math.SubtractVV(b2Math.AddVV(r.localPosition, i.center), this.m_center);
			...
			this.m_linearVelocity = b2Math.AddVV(e.linearVelocity, b2Math.b2CrossFV(e.angularVelocity, this.m_center));
	...
	b2CollisionFilter.prototype = {
		ShouldCollide: function(e, t) {
			...
			var n = (e.m_maskBits & t.m_categoryBits) != 0 && (e.m_categoryBits & t.m_maskBits) != 0;
			return n
		},
		initialize: function() {}
	};
	...
	b2Island.prototype = {
		...
		Solve: function(e, t) {
			...
			for (n = 0; n < this.m_bodyCount; ++n) {
				...
				r.m_linearVelocity.Add(b2Math.MulFV(e.dt, b2Math.AddVV(t, b2Math.MulFV(r.m_invMass, r.m_force))));
			...
			var i = new b2ContactSolver(this.m_contacts, this.m_contactCount, this.m_allocator);
			i.PreSolve();
			...
			if (b2World.s_enablePositionCorrection) {
				for (b2Island.m_positionIterationCount = 0; b2Island.m_positionIterationCount < e.iterations; ++b2Island.m_positionIterationCount) {
		...
		UpdateSleep: function(e) {
			...
			var i = b2Settings.b2_linearSleepTolerance * b2Settings.b2_linearSleepTolerance;
			var s = b2Settings.b2_angularSleepTolerance * b2Settings.b2_angularSleepTolerance;
			for (t = 0; t < this.m_bodyCount; ++t) {
				...
				if ((n.m_flags & b2Body.e_allowSleepFlag) == 0 || n.m_angularVelocity * n.m_angularVelocity > s || b2Math.b2Dot(n.m_linearVelocity, n.m_linearVelocity) > i) {
					n.m_sleepTime = 0;
					r = 0
				} else {
					n.m_sleepTime += e;
...
this.m_restitution = b2Math.b2Max(this.m_shape1.m_restitution, this.m_shape2.m_restitution); 
		...
		SolvePositionConstraints: function(e) {
			...
			for (var i = 0; i < this.m_constraintCount; ++i) {
				...
				for (var E = 0; E < w; ++E) {
					...
					t = b2Math.b2Min(t, D);
					var P = e * b2Math.b2Clamp(D + b2Settings.b2_linearSlop, -b2Settings.b2_maxLinearCorrection, 0);
	...
	Object.extend(b2ContactManager.prototype, {
		...
		PairAdded: function(e, t) {
			...
			if (this.m_world.m_filter != null && this.m_world.m_filter.ShouldCollide(n, r) == false) {
				return this.m_nullContact
			}
			if (s.m_invMass == 0) {
				var o = n;
	...
	b2World.prototype = {
		...
		Step: function(e, t) {
			...
			var i = new b2Island(this.m_bodyCount, this.m_contactCount, this.m_jointCount, this.m_stackAllocator);
			...
			for (var l = this.m_bodyList; l != null; l = l.m_next) {
				if (l.m_flags & (b2Body.e_staticFlag | b2Body.e_islandFlag | b2Body.e_sleepFlag | b2Body.e_frozenFlag)) {
					continue
				}
				i.Clear();
				var c = 0;
	...
	Object.extend(b2DistanceJoint.prototype, {
		...
		PrepareVelocitySolver: function() {
			var e;
			e = this.m_body1.m_R;
			var t = e.col1.x * this.m_localAnchor1.x + e.col2.x * this.m_localAnchor1.y;
			var n = e.col1.y * this.m_localAnchor1.x + e.col2.y * this.m_localAnchor1.y;
			e = this.m_body2.m_R;
			var r = e.col1.x * this.m_localAnchor2.x + e.col2.x * this.m_localAnchor2.y;
			var i = e.col1.y * this.m_localAnchor2.x + e.col2.y * this.m_localAnchor2.y;
			this.m_u.x = this.m_body2.m_position.x + r - this.m_body1.m_position.x - t;
			this.m_u.y = this.m_body2.m_position.y + i - this.m_body1.m_position.y - n;
			...
			this.m_mass = this.m_body1.m_invMass + this.m_body1.m_invI * o * o + this.m_body2.m_invMass + this.m_body2.m_invI * u * u;
		...
		SolveVelocityConstraints: function(e) {
			var t;
			t = this.m_body1.m_R;
			var n = t.col1.x * this.m_localAnchor1.x + t.col2.x * this.m_localAnchor1.y;
			var r = t.col1.y * this.m_localAnchor1.x + t.col2.y * this.m_localAnchor1.y;
			t = this.m_body2.m_R;
			var i = t.col1.x * this.m_localAnchor2.x + t.col2.x * this.m_localAnchor2.y;
			var s = t.col1.y * this.m_localAnchor2.x + t.col2.y * this.m_localAnchor2.y;
			var o = this.m_body1.m_linearVelocity.x + -this.m_body1.m_angularVelocity * r;
			var u = this.m_body1.m_linearVelocity.y + this.m_body1.m_angularVelocity * n;
			var a = this.m_body2.m_linearVelocity.x + -this.m_body2.m_angularVelocity * s;
			var f = this.m_body2.m_linearVelocity.y + this.m_body2.m_angularVelocity * i;
	...
	b2Jacobian.prototype = {
		...
		Compute: function(e, t, n, r) {
			return this.linear1.x * e.x + this.linear1.y * e.y + this.angular1 * t + (this.linear2.x * n.x + this.linear2.y * n.y) + this.angular2 * r
		},
		initialize: function() {
	...
	Object.extend(b2GearJoint.prototype, {
		GetAnchor1: function() {
			...
			return new b2Vec2(this.m_body1.m_position.x + (e.col1.x * this.m_localAnchor1.x + e.col2.x * this.m_localAnchor1.y), this.m_body1.m_position.y + (e.col1.y * this.m_localAnchor1.x + e.col2.y * this.m_localAnchor1.y))
		},
		GetAnchor2: function() {
			var e = this.m_body2.m_R;
			return new b2Vec2(this.m_body2.m_position.x + (e.col1.x * this.m_localAnchor2.x + e.col2.x * this.m_localAnchor2.y), this.m_body2.m_position.y + (e.col1.y * this.m_localAnchor2.x + e.col2.y * this.m_localAnchor2.y))
		},
		GetReactionForce: function(e) {
			return new b2Vec2
		},
		...
		SolveVelocityConstraints: function(e) {
			var t = this.m_body1;
			var n = this.m_body2;
			var r = this.m_J.Compute(t.m_linearVelocity, t.m_angularVelocity, n.m_linearVelocity, n.m_angularVelocity);
			var i = -this.m_mass * r;
			this.m_impulse += i;
	...
	Object.extend(b2MouseJoint.prototype, {
		...
		initialize: function(e) {
			...
			this.m_localAnchor.x = t * this.m_body2.m_R.col1.x + n * this.m_body2.m_R.col1.y;
			this.m_localAnchor.y = t * this.m_body2.m_R.col2.x + n * this.m_body2.m_R.col2.y;
		...
		SolveVelocityConstraints: function(e) {
			...
			n = this.m_ptpMass;
			var u = s + this.m_beta * e.inv_dt * this.m_C.x + this.m_gamma * this.m_impulse.x;
			var a = o + this.m_beta * e.inv_dt * this.m_C.y + this.m_gamma * this.m_impulse.y;
	...
	Object.extend(b2PrismaticJoint.prototype, {
		...
		GetJointTranslation: function() {
			...
			var n;
			n = e.m_R;
			var r = n.col1.x * this.m_localAnchor1.x + n.col2.x * this.m_localAnchor1.y;
			var i = n.col1.y * this.m_localAnchor1.x + n.col2.y * this.m_localAnchor1.y;
			n = t.m_R;
			var s = n.col1.x * this.m_localAnchor2.x + n.col2.x * this.m_localAnchor2.y;
			var o = n.col1.y * this.m_localAnchor2.x + n.col2.y * this.m_localAnchor2.y;
		...
		GetJointSpeed: function() {
			...
			var b = c * -g * d + h * g * p + (p * (m.x + -y * o - v.x - -g * i) + d * (m.y + y * s - v.y - g * r));
			return b
		},
		...
		GetReactionForce: function(e) {
			...
			var n;
			n = this.m_body1.m_R;
			var r = t * (n.col1.x * this.m_localXAxis1.x + n.col2.x * this.m_localXAxis1.y);
			var i = t * (n.col1.y * this.m_localXAxis1.x + n.col2.y * this.m_localXAxis1.y);
			var s = t * (n.col1.x * this.m_localYAxis1.x + n.col2.x * this.m_localYAxis1.y);
			var o = t * (n.col1.y * this.m_localYAxis1.x + n.col2.y * this.m_localYAxis1.y);
		...
		initialize: function(e) {
			...
			this.m_localAnchor1.Set(n * t.col1.x + r * t.col1.y, n * t.col2.x + r * t.col2.y);
			t = this.m_body2.m_R;
			n = e.anchorPoint.x - this.m_body2.m_position.x;
			r = e.anchorPoint.y - this.m_body2.m_position.y;
			this.m_localAnchor2.Set(n * t.col1.x + r * t.col1.y, n * t.col2.x + r * t.col2.y);
			t = this.m_body1.m_R;
			n = e.axis.x;
			r = e.axis.y;
			this.m_localXAxis1.Set(n * t.col1.x + r * t.col1.y, n * t.col2.x + r * t.col2.y);
		...
		PrepareVelocitySolver: function() {
			...
			this.m_linearMass = u + f * this.m_linearJacobian.angular1 * this.m_linearJacobian.angular1 + a + l * this.m_linearJacobian.angular2 * this.m_linearJacobian.angular2;
			this.m_linearMass = 1 / this.m_linearMass;
			this.m_angularMass = 1 / (f + l);
			if (this.m_enableLimit || this.m_enableMotor) {
				n = e.m_R;
				var v = n.col1.x * this.m_localXAxis1.x + n.col2.x * this.m_localXAxis1.y;
				var m = n.col1.y * this.m_localXAxis1.x + n.col2.y * this.m_localXAxis1.y;
				...
				this.m_motorMass = u + f * this.m_motorJacobian.angular1 * this.m_motorJacobian.angular1 + a + l * this.m_motorJacobian.angular2 * this.m_motorJacobian.angular2;
			...
			if (b2World.s_enableWarmStarting) {
				var w = this.m_linearImpulse * this.m_linearJacobian.linear1.x + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear1.x;
				var E = this.m_linearImpulse * this.m_linearJacobian.linear1.y + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear1.y;
				var S = this.m_linearImpulse * this.m_linearJacobian.linear2.x + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear2.x;
				...
				var T = this.m_linearImpulse * this.m_linearJacobian.angular1 - this.m_angularImpulse + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.angular1;
				var N = this.m_linearImpulse * this.m_linearJacobian.angular2 + this.m_angularImpulse + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.angular2;
		...
		SolveVelocityConstraints: function(e) {
			...
			var o = n.m_invI;
			var u;
			var a = this.m_linearJacobian.Compute(t.m_linearVelocity, t.m_angularVelocity, n.m_linearVelocity, n.m_angularVelocity);
			...
			if (this.m_enableMotor && this.m_limitState != b2Joint.e_equalLimits) {
				var h = this.m_motorJacobian.Compute(t.m_linearVelocity, t.m_angularVelocity, n.m_linearVelocity, n.m_angularVelocity) - this.m_motorSpeed;
				var p = -this.m_motorMass * h;
				var d = this.m_motorImpulse;
				this.m_motorImpulse = b2Math.b2Clamp(this.m_motorImpulse + p, -e.dt * this.m_maxMotorForce, e.dt * this.m_maxMotorForce);
			...
			if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
				var v = this.m_motorJacobian.Compute(t.m_linearVelocity, t.m_angularVelocity, n.m_linearVelocity, n.m_angularVelocity);
		...
		SolvePositionConstraints: function() {
			...
			var a;
			a = n.m_R;
			var f = a.col1.x * this.m_localAnchor1.x + a.col2.x * this.m_localAnchor1.y;
			var l = a.col1.y * this.m_localAnchor1.x + a.col2.y * this.m_localAnchor1.y;
			a = r.m_R;
			var c = a.col1.x * this.m_localAnchor2.x + a.col2.x * this.m_localAnchor2.y;
			var h = a.col1.y * this.m_localAnchor2.x + a.col2.y * this.m_localAnchor2.y;
			...
			var E = b * g + w * y;
			E = b2Math.b2Clamp(E, -b2Settings.b2_maxLinearCorrection, b2Settings.b2_maxLinearCorrection);
			...
			T = b2Math.b2Clamp(T, -b2Settings.b2_maxAngularCorrection, b2Settings.b2_maxAngularCorrection);
			...
			if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
				...
				y = m - d;
				a = n.m_R;
				var k = a.col1.x * this.m_localXAxis1.x + a.col2.x * this.m_localXAxis1.y;
				var L = a.col1.y * this.m_localXAxis1.x + a.col2.y * this.m_localXAxis1.y;
				var A = k * g + L * y;
				var O = 0;
				if (this.m_limitState == b2Joint.e_equalLimits) {
					e = b2Math.b2Clamp(A, -b2Settings.b2_maxLinearCorrection, b2Settings.b2_maxLinearCorrection);
					O = -this.m_motorMass * e;
					x = b2Math.b2Max(x, b2Math.b2Abs(T))
				} else if (this.m_limitState == b2Joint.e_atLowerLimit) {
					e = A - this.m_lowerTranslation;
					x = b2Math.b2Max(x, -e);
					e = b2Math.b2Clamp(e + b2Settings.b2_linearSlop, -b2Settings.b2_maxLinearCorrection, 0);
					O = -this.m_motorMass * e;
					t = this.m_limitPositionImpulse;
					this.m_limitPositionImpulse = b2Math.b2Max(this.m_limitPositionImpulse + O, 0);
					O = this.m_limitPositionImpulse - t
				} else if (this.m_limitState == b2Joint.e_atUpperLimit) {
					e = A - this.m_upperTranslation;
					x = b2Math.b2Max(x, e);
					e = b2Math.b2Clamp(e - b2Settings.b2_linearSlop, 0, b2Settings.b2_maxLinearCorrection);
					O = -this.m_motorMass * e;
					t = this.m_limitPositionImpulse;
					this.m_limitPositionImpulse = b2Math.b2Min(this.m_limitPositionImpulse + O, 0);
					O = this.m_limitPositionImpulse - t
				}
	...
	Object.extend(b2PulleyJoint.prototype, {
		...
		GetLength1: function() {
			var e;
			e = this.m_body1.m_R;
			var t = this.m_body1.m_position.x + (e.col1.x * this.m_localAnchor1.x + e.col2.x * this.m_localAnchor1.y);
			var n = this.m_body1.m_position.y + (e.col1.y * this.m_localAnchor1.x + e.col2.y * this.m_localAnchor1.y);
		...
		GetLength2: function() {
			var e;
			e = this.m_body2.m_R;
			var t = this.m_body2.m_position.x + (e.col1.x * this.m_localAnchor2.x + e.col2.x * this.m_localAnchor2.y);
			var n = this.m_body2.m_position.y + (e.col1.y * this.m_localAnchor2.x + e.col2.y * this.m_localAnchor2.y);
		...
		initialize: function(e) {
			...
			var o = b2Math.b2Max(.5 * b2PulleyJoint.b2_minPulleyLength, i);
			var u = b2Math.b2Max(.5 * b2PulleyJoint.b2_minPulleyLength, s);
			this.m_constant = o + this.m_ratio * u;
			this.m_maxLength1 = b2Math.b2Clamp(e.maxLength1, o, this.m_constant - this.m_ratio * b2PulleyJoint.b2_minPulleyLength);
			this.m_maxLength2 = b2Math.b2Clamp(e.maxLength2, u, (this.m_constant - b2PulleyJoint.b2_minPulleyLength) / this.m_ratio);
		...
		PrepareVelocitySolver: function() {
			...
			var E = (-this.m_ratio * this.m_pulleyImpulse - this.m_limitImpulse2) * this.m_u2.x;
			var S = (-this.m_ratio * this.m_pulleyImpulse - this.m_limitImpulse2) * this.m_u2.y;
		...
		SolveVelocityConstraints: function(e) {
			...
			var r;
			r = t.m_R;
			var i = r.col1.x * this.m_localAnchor1.x + r.col2.x * this.m_localAnchor1.y;
			var s = r.col1.y * this.m_localAnchor1.x + r.col2.y * this.m_localAnchor1.y;
			r = n.m_R;
			var o = r.col1.x * this.m_localAnchor2.x + r.col2.x * this.m_localAnchor2.y;
			var u = r.col1.y * this.m_localAnchor2.x + r.col2.y * this.m_localAnchor2.y;
			var a;
			var f;
			var l;
			var c;
			...
			m = -(this.m_u1.x * a + this.m_u1.y * f) - this.m_ratio * (this.m_u2.x * l + this.m_u2.y * c);
		...
		SolvePositionConstraints: function() {
			...
			var w = 0; {
				...
				g = b2Math.b2Clamp(g, -b2Settings.b2_maxLinearCorrection, b2Settings.b2_maxLinearCorrection);
			...
			if (this.m_limitState1 == b2Joint.e_atUpperLimit) {
				...
				w = b2Math.b2Max(w, -g);
				g = b2Math.b2Clamp(g + b2Settings.b2_linearSlop, -b2Settings.b2_maxLinearCorrection, 0);
				y = -this.m_limitMass1 * g;
				b = this.m_limitPositionImpulse1;
				this.m_limitPositionImpulse1 = b2Math.b2Max(0, this.m_limitPositionImpulse1 + y);
			...
			if (this.m_limitState2 == b2Joint.e_atUpperLimit) {
				...
				this.m_limitPositionImpulse2 = b2Math.b2Max(0, this.m_limitPositionImpulse2 + y);
	...
	Object.extend(b2RevoluteJoint.prototype, {
		...
		PrepareVelocitySolver: function() {
			...
			if (b2World.s_enableWarmStarting) {
				...
				e.m_angularVelocity -= f * (r * this.m_ptpImpulse.y - i * this.m_ptpImpulse.x + this.m_motorImpulse + this.m_limitImpulse);
				t.m_linearVelocity.x += a * this.m_ptpImpulse.x;
				t.m_linearVelocity.y += a * this.m_ptpImpulse.y;
				t.m_angularVelocity += l * (s * this.m_ptpImpulse.y - o * this.m_ptpImpulse.x + this.m_motorImpulse + this.m_limitImpulse)
			} else {
		...
		SolveVelocityConstraints: function(e) {
			...
			var u = r.col1.y * this.m_localAnchor2.x + r.col2.y * this.m_localAnchor2.y;
			var a;
			var f = n.m_linearVelocity.x + -n.m_angularVelocity * u - t.m_linearVelocity.x - -t.m_angularVelocity * s;
			var l = n.m_linearVelocity.y + n.m_angularVelocity * o - t.m_linearVelocity.y - t.m_angularVelocity * i;
			...
			if (this.m_enableMotor && this.m_limitState != b2Joint.e_equalLimits) {
				...
				this.m_motorImpulse = b2Math.b2Clamp(this.m_motorImpulse + d, -e.dt * this.m_maxMotorTorque, e.dt * this.m_maxMotorTorque);
		...
		SolvePositionConstraints: function() {
			...
			var s;
			s = n.m_R;
			var o = s.col1.x * this.m_localAnchor1.x + s.col2.x * this.m_localAnchor1.y;
			var u = s.col1.y * this.m_localAnchor1.x + s.col2.y * this.m_localAnchor1.y;
			s = r.m_R;
			var a = s.col1.x * this.m_localAnchor2.x + s.col2.x * this.m_localAnchor2.y;
			var f = s.col1.y * this.m_localAnchor2.x + s.col2.y * this.m_localAnchor2.y;
			...
			if (this.m_enableLimit && this.m_limitState != b2Joint.e_inactiveLimit) {
				...
				var T = 0;
				if (this.m_limitState == b2Joint.e_equalLimits) {
					t = b2Math.b2Clamp(x, -b2Settings.b2_maxAngularCorrection, b2Settings.b2_maxAngularCorrection);
					T = -this.m_motorMass * t;
					S = b2Math.b2Abs(t)
				} else if (this.m_limitState == b2Joint.e_atLowerLimit) {
					t = x - this.m_lowerAngle;
					S = b2Math.b2Max(0, -t);
					t = b2Math.b2Clamp(t + b2Settings.b2_angularSlop, -b2Settings.b2_maxAngularCorrection, 0);
					T = -this.m_motorMass * t;
					e = this.m_limitPositionImpulse;
					this.m_limitPositionImpulse = b2Math.b2Max(this.m_limitPositionImpulse + T, 0);
					T = this.m_limitPositionImpulse - e
				} else if (this.m_limitState == b2Joint.e_atUpperLimit) {
					t = x - this.m_upperAngle;
					S = b2Math.b2Max(0, t);
					t = b2Math.b2Clamp(t - b2Settings.b2_angularSlop, 0, b2Settings.b2_maxAngularCorrection);
					T = -this.m_motorMass * t;
					e = this.m_limitPositionImpulse;
					this.m_limitPositionImpulse = b2Math.b2Min(this.m_limitPositionImpulse + T, 0);
					T = this.m_limitPositionImpulse - e
				}
						...
						if (c !== "parent")
							...
							if (!p.is("body")) {
								...
								this.stage = {
									X: p.offset().left,
									Y: p.offset().top,
									Width: p.offset().left + p.outerWidth(),
									Height: p.offset().top + p.outerHeight() - area
								};
							}
